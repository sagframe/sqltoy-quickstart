...
public class OrganInfoServiceImpl implements OrganInfoService {
	...
	public void saveOrganInfo(OrganInfoVO organInfoVO) {
		...
		// sqlToyLazyDao.wrapTreeTableRoute(new
		// TreeTableModel(organInfoVO).idField("organCode").pidField("organPid"));
		// idField 如果是主键则无需设置
		lightDao.wrapTreeTableRoute(new TreeTableModel(organInfoVO).idField("organCode").pidField("organPid"));
	}
